...
######################################################################################
EXAMPLE = """example:
  prep_geodmod.py $SCRATCHDIR/DarbandikhanSenAT72/PYSAR/timeseries_ECMWF_demErr.h5 20171117_20180603 -b 34.2 35.2 45.0 46.3 -y 0.001 -x 0.001 -t disp -o AT20171117_20180603.unw 
  prep_geodmod.py $SCRATCHDIR/DarbandikhanSenAT72/PYSAR/INPUTS/ifgramStack.h5 20180603_20180615 -b 34.2 35.2 45.0 46.3 -y 0.001 -x 0.001 -t cor -o AT20180603_20180615.cor
  prep_geodmod.py $SCRATCHDIR/DarbandikhanSenAT72/PYSAR/temporalCoherence.h5 20171117_20180603 -b 34.2 35.2 45.0 46.3 -y 0.001 -x 0.001 -t cor -o AT20171117_20180603.cor
  prep_geodmod.py $SCRATCHDIR/DarbandikhanSenAT72/PYSAR/INPUTS/geometryRadar.h5 20171117_20180603 -b 34.2 35.2 45.0 46.3 -y 0.001 -x 0.001 -t dem -o AT.dem
...
def create_parser():
    ...
    parser.add_argument('-t','--type',dest='Type',type=str,
                        help='The type of data with choice [disp, cor, dem]')
...
def write_rsc_file(in_file,out_file):
    """ write rsc file for Geodmod"""
    # read file
    meta = readfile.read_roipac_rsc(in_file)
    # initiate dict
    rsc = dict()
    rsc['FILE_DIR'] = 'lxr/Iraq/afterslip_example'
...
def main(iargs=None):
    ...
    if inps.Type=='disp':
        ...
        asc_args = [atr_asc, '-b',inps.SNWE, '-y',inps.latStep, '-x',inps.lonStep]
        print("geocode.py", asc_args)
        args_str = format_args(asc_args)
        mintpy.geocode.main(args_str.split())
    
        _, shotname, extension = get_path_com(atr_asc)
        asct_args = ['geo_'+shotname+extension, inps.dset[0], '-o', inps.outfile]
        ...
        _, shotname, extension = get_path_com(atr_asc)
        if shotname=='ifgramStack':
            asct_args = ['geo_'+shotname+extension, 'coherence-'+inps.dset[0], '-o', inps.outfile]
        else:        
            asct_args = ['geo_'+shotname+extension, '-o', inps.outfile]
        print("save_roipac.py", asct_args)
        asct_str = format_args(asct_args)
        mintpy.save_roipac.main(asct_str.split())
    else:
        ...
        _, shotname, extension = get_path_com(atr_asc)
        asct_args = ['geo_'+shotname+extension, 'height', '-o', inps.outfile]
        print("save_roipac.py", asct_args)
        asct_str = format_args(asct_args)
        mintpy.save_roipac.main(asct_str.split())
        ...
        print(rscdir)        
        os.chdir(rscdir)
        write_rsc_file(outfile,format_args([format_args(inps.outfile) +'.rsc1']))
